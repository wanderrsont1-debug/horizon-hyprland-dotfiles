...
# The Horizon Arch Offline ISO Factory
...
cp -r /usr/share/archiso/configs/releng /mnt/zram1/horizon_iso/profile
...
mkdir -p /mnt/zram1/horizon_iso/profile/airootfs/root/arch_install
```
...
cp -a ~/user_scripts/arch_iso_scripts/offline_iso/* /mnt/zram1/horizon_iso/profile/airootfs/root/arch_install/
...
cp -a ~/user_scripts/arch_iso_scripts/offline_iso/iso_maker/030_build_iso.sh /mnt/zram1/horizon_iso/
```
...
*Once Step 4 finishes successfully, your final `.iso` file will be waiting in `/mnt/zram1/horizon_iso/out/`.*
...
sudo dd if=/mnt/zram1/horizon_iso/out/archlinux-*.iso of=/dev/sdX bs=4M status=progress oflag=sync
```