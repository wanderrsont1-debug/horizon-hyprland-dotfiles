...
Before you configure, understand _why_ Horizon works the way it does.

### The "Restaurant" Analogy

To understand how the configuration works, think of Horizon Control Center like a high-end restaurant:

1. **The YAML (`horizon_config.yaml`) is the Menu.** It lists what is available, the prices, and the descriptions. It doesn't cook the food; it just describes it.
    
2. **The Python Backend (`rows.py`) is the Kitchen.** It takes the order and actually prepares the widget (buttons, sliders, toggles).
    
3. **The Daemon Mode.** Unlike most apps that "close" (shut down the kitchen) when you click 'X', Horizon just turns off the dining room lights. The kitchen staff stays ready in the background. When you open it again, service is instant.
...
>     
> - **UWSM Compliance (The Health Inspector):** Apps launched by Horizon are wrapped in `uwsm-app`. This ensures they know they are running in a Hyprland session and don't crash or become "zombies" when you close the Control Center.
>     
> - **Hot Reload (Printing New Menus):** When you save the YAML and hit `Ctrl+R`, Horizon instantly prints a new menu and reorganizes the tables without kicking the customers (you) out.
>     

## 2. Configuration Structure

The configuration file is located at `$HOME/user_scripts/horizon_config.yaml`.

The hierarchy is strict. If the indentation is wrong, the "Menu" is unreadable.

**Pages** $\to$ **Layouts** (Sections) $\to$ **Items** (Widgets)

The `horizon_config.yaml` is the heart of the system with this hierarchical structure:
...
- `key`: Alternatively, link to a file in `~/.config/horizon/settings/` for internal state.
...
- type: toggle_card
  properties:
    ...
    key: horizon_theme/state # Saves state to file
  on_toggle:
    enabled:
      type: exec
...
1. Edit `horizon_config.yaml`.
    
2. Focus the Horizon window.
...
> State Preservation
> 
> Horizon attempts to remember exactly which page you were on. However, if you delete that page in the config, it will default back to the first page.

### 4.2 Deep Search (`Ctrl+F`)

**Analogy:** The Index.

Horizon includes a recursive indexing engine that finds needles in haystacks.
...
- Clicking the "Horizon" icon in the top left or the sidebar toggle button will slide the menu in/out.
...
- **Explanation:** You clicked the toggle, the command ran, but the system (e.g., NetworkManager) took 1 second to actually change status. The Horizon Monitor checked instantly, saw the old status, and flipped the switch back to match reality.
...
### "My command works in terminal but not in Horizon"

- **Cause:** Path issues.
    
- **Fix:** Always use absolute paths (e.g., `/usr/bin/htop` instead of `htop`). While Horizon handles `$HOME` expansion, it does not automatically load your `.bashrc` aliases.
...
- **Recover:** If the config is broken, Horizon will launch into a special "Error State" page showing you the Python stack trace, allowing you to fix the YAML and Hot Reload (`Ctrl+R`) without crashing.
...
### 4. `horizon_config.yaml` (The Menu)
...
- **Key Job:** Makes the app look like "Horizon" rather than a generic Linux window.