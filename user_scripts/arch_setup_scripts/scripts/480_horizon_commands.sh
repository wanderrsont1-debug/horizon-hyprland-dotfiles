...
# ==============================================================================
#  HORIZON FLEET PATCHER (Enterprise Edition - V3)
...
declare -ra FLEET_COMMANDS=(
 ...
 #   'U | gsettings set org.cinnamon.desktop.default-applications.terminal exec "kitty"'
    ...
    'U | mkdir -p "$HOME/Documents/horizon_backups/"'
    'U | TARGET="$HOME/user_scripts/horizon_system/click_away_to_dismiss" && \
          ...
          gcc -shared -fPIC -o "$TARGET/libwaylandgrab.so" "$TARGET/horizon.c" "$TARGET/hyprland-focus-grab-v1-client-protocol.c" -lwayland-client -lpthread -ldl'

    'U | systemctl --user daemon-reload && systemctl --user restart horizon_quickpanal.service || true'
    'U | "$HOME/user_scripts/horizon_system/reload_cc/cc_restart.sh" --quiet &'
    'U | "$HOME/user_scripts/horizon_system/quickpanals/reload_quickpanal.sh/" --quiet &'
    # --- System Services ---
#    "U | systemctl --user disable horizon.service || true"
...
# 2. Paths & Constants
readonly STATE_DIR="${HOME}/.local/state/horizon"
readonly STATE_FILE="${STATE_DIR}/patch_history.state"
readonly LOG_FILE="${HOME}/Documents/logs/horizon_patcher_$(date +%Y%m%d_%H%M%S).log"
readonly LOCK_FILE="${XDG_RUNTIME_DIR:-/run/user/$UID}/horizon_fleet_patcher.lock"
