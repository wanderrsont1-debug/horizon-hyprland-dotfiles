#!/usr/bin/env bash
# ==============================================================================
# DUSKY INPUT :: MODERN WAYLAND AUDIO SWITCHER
...
for cmd in wpctl rofi notify-send awk; do
    if ! command -v "$cmd" &>/dev/null; then
        notify-send -u critical "Dusky Input" "Missing required dependency: $cmd"
        exit 1
    fi
done
...
if [[ ${#rofi_options[@]} -eq 0 ]]; then
    notify-send -u critical "Dusky Input" "No input devices found."
    exit 1
fi
