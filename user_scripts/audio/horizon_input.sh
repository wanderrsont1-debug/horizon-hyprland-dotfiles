#!/usr/bin/env bash
# ==============================================================================
# HORIZON INPUT :: MODERN WAYLAND AUDIO SWITCHER
...
for cmd in wpctl rofi notify-send awk; do
    if ! command -v "$cmd" &>/dev/null; then
        notify-send -u critical "Horizon Input" "Missing required dependency: $cmd"
        exit 1
    fi
done
...
# 2. Parse native WirePlumber status block for Sources (Inputs)
...
done < <(wpctl status | awk '/Sources:/{f=1; next} /Filters:|Streams:|Settings:/{f=0} f')

if [[ ${#rofi_options[@]} -eq 0 ]]; then
    notify-send -u critical "Horizon Input" "No input devices found."
    exit 1
fi
...
if [[ -n "$choice" && -n "${device_map[$choice]:-}" ]]; then
    ...
    osd_icon="microphone-sensitivity-high-symbolic"
