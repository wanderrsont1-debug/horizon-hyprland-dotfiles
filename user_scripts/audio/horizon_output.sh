#!/usr/bin/env bash
# ==============================================================================
# HORIZON OUTPUT :: MODERN WAYLAND AUDIO SWITCHER
...
for cmd in wpctl rofi notify-send awk; do
    if ! command -v "$cmd" &>/dev/null; then
        notify-send -u critical "Horizon Output" "Missing required dependency: $cmd"
        exit 1
    fi
done
...
if [[ ${#rofi_options[@]} -eq 0 ]]; then
    notify-send -u critical "Horizon Output" "No output devices found."
    exit 1
fi
