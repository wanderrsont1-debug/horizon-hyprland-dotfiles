#!/usr/bin/env python3
"""
Horizon BTRFS & Snapper Master Controller
...
def setup_logger() -> logging.Logger:
    ...
    """
    logger = logging.getLogger("horizon-master")
    ...
    try:
        ...
        syslog_handler.setFormatter(logging.Formatter('horizon-master[%(process)d]: %(levelname)s - %(message)s'))
    ...
    try:
        file_handler = logging.FileHandler("/var/log/horizon.log")
        ...
        for plan in plans:
            ...
            if not deleted:
                ...
                try:
                    ...
                    service_name = f"horizon-cleanup-{subvol_name}.service"
...
Description=Horizon Btrfs Cleanup ({subvol_name})
After=local-fs.target

[Service]
Type=oneshot
ExecStart=/usr/bin/bash -c "/usr/bin/mkdir -p /run/horizon_mnt && /usr/bin/mount -t btrfs -o subvolid=5 UUID={uuid} /run/horizon_mnt && if [ ! -e '/run/horizon_mnt/{subvol_name}' ]; then /usr/bin/umount /run/horizon_mnt; exit 0; elif /usr/bin/btrfs subvolume delete '/run/horizon_mnt/{subvol_name}'; then /usr/bin/umount /run/horizon_mnt; else /usr/bin/umount /run/horizon_mnt; exit 1; fi"
...
def launch_tui() -> None:
    ...
    while True:
        ...
        fzf_cmd = [
            ...
            "--border=rounded", "--border-label", " Horizon Snapshots ",
