...
"""
Horizon Formatter v5.3.3 (Architect Edition - Patched)
...
if os.geteuid() != 0:
    print("\033[1;33m[!] Horizon Formatter requires root privileges. Elevating via sudo...\033[0m")
    try:
...
def generate_secure_mapper_name() -> str:
    return f"horizon_luks_{uuid.uuid4().hex[:8]}"

def interactive_setup() -> FormatPlan:
    console.print(Panel.fit("[bold magenta]Horizon Formatter v5.3.3[/] - [cyan]Arch Linux Storage & Analysis Utility[/]", border_style="magenta"))
    ...
    while True:
        ...
        if active_mounts or active_mappings:
            ...
            if Confirm.ask("Would you like Horizon Formatter to attempt a [bold red]force unlock & unmount[/] now?", default=False):
...
def execute_plan(commands: list[ExecutionStep], mapper_name: Optional[str] = None) -> None:
    console.print("\n[bold cyan]Executing Horizon Formatting Plan...[/]")
    luks_is_open = False
