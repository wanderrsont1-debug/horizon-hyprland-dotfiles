...
declare -r APP_TITLE="Horizon Firefox Themer"
declare -r APP_VERSION="v1.6.1 (Local)"

declare -r THEME_DIR="$HOME/.config/horizon_sites"
...
deploy_changes() {
    ...
    local menu_css="$chrome_dir/horizon_menu.css"
    cat << 'EOF' > "$menu_css"
/* Auto-generated by Horizon TUI - Do not edit manually */
menupopup {
...
EOF
    ...
    printf '@import url("horizon_menu.css");\n\n' >> "$tmp_chrome"
    
    # Copy the rest of the users custom CSS, stripping out old duplicates of the imports
    grep -vE '^[[:space:]]*@import url\("?(colors\.css|horizon_menu\.css)"?\);' "$user_chrome" >> "$tmp_chrome" || true
