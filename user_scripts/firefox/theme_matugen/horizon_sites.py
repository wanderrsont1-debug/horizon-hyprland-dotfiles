#!/usr/bin/env python3
"""
Horizon Dynamic Theme Builder - Ultimate Live Preview Master Edition
...
try:
    ...
    if os.environ.get("_HORIZON_BOOTSTRAP_ATTEMPTED"):
    ...
    try:
        ...
        os.environ["_HORIZON_BOOTSTRAP_ATTEMPTED"] = "1"
...
CONFIG_DIR = get_xdg_config_home() / "horizon_sites"
    ...
    def inject_rules(self, new_rules: list[dict[str, Any]]):
        ...
        for r in inner_rules:
            if getattr(r, 'type', '') == 'qualified-rule':
                ...
                meta_decl = next((d for d in decls if d.lower_name == '--horizon-meta'), None)
        ...
        for r_data in new_rules:
            ...
            if map_key in existing_rules_map:
                ...
                css_lines = [f"{sel} {{"]
                if meta:
                    css_lines.append(f"        --horizon-meta: \"{meta}\";")
                for k, v in props:
                    suffix = " !important" if "!important" not in str(v).lower() else ""
...
def extract_css_variables(text: str) -> list[str]:
    ...
    return list(dict.fromkeys([m for m in matches if m != '--horizon-meta']))
...
def flow_audit_mode():
    console.clear()
    console.print(Panel.fit("=== Horizon Auditor: Fix or Prune Selectors ===", style="bold yellow"))
    
    CONFIG_DIR.mkdir(parents=True, exist_ok=True)
    css_files = sorted(CONFIG_DIR.glob("*.css"), key=lambda f: f.name)
    
    if not css_files:
        console.print("[bold red]No themes found in your Horizon Sites config directory.[/]")
    ...
    manager = HorizonASTManager(domain, selected_file)
...
def flow_create_edit():
    console.clear()
    console.print(Panel.fit("=== Horizon Dynamic Editor (Live Preview Mode) ===", style="bold magenta"))
    ...
    manager = HorizonASTManager(domain, file_path)
    ...
    if deploy_choice == "y":
        ...
        tui_script = scripts_dir / "horizon_firefox_tui.sh"
...
def main():
    while True:
        ...
        console.print(Panel.fit(
            "[bold cyan]Horizon Wayland CSS Generator[/] (Live Master Edition)\n"
