#!/usr/bin/env bash
# =============================================================================
# Horizon Git Time Machine (Platinum Edition - Architecture v9.2 - Ephemeral Zenith)
...
_horizon_volatile_shift() {
    ...
    if [[ -d "$zram_dir" && -w "$zram_dir" ]]; then
        if touch "${zram_dir}/.horizon_write_test" 2>/dev/null; then
            target_dir="$zram_dir"
            rm -f "${zram_dir}/.horizon_write_test"
        fi
    fi

    # If we are already running inside the volatile zone, bypass relocation and proceed.
    if [[ "$current_path" == "${target_dir}/_horizon_tm_run_"* ]]; then
        return 0
    fi

    # We are in the volatile timeline (Git work tree). Clone to RAM and jump.
    local -r volatile_path="${target_dir}/_horizon_tm_run_${$}_${RANDOM}.sh"
...
_horizon_cleanup() {
    ...
    if [[ -f "/tmp/horizon_time_machine_stash_${HORIZON_SESSION_ID}" ]]; then
        _horizon_git_return >/dev/null 2>&1
    fi
    rm -f "/tmp/horizon_time_machine_stash_${HORIZON_SESSION_ID}" 2>/dev/null || true

    # 2. Ephemeral Instance Self-Destruct (Cleans up the RAM clone)
    local -r current_path="$(cd "$(dirname "${BASH_SOURCE[0]}")" &>/dev/null && pwd)/$(basename "${BASH_SOURCE[0]}")"
    if [[ "$current_path" == *"/_horizon_tm_run_"* ]]; then
        rm -f "$current_path" 2>/dev/null || true
    fi
}
# Trap EXIT fires reliably in Bash 5+ for normal exits, Ctrl+C (SIGINT), and errors.
trap _horizon_cleanup EXIT
...
_horizon_git_help() {
    clear
    printf "\n\n  \033[1;38;5;81m󰏖 Horizon Time Machine - Keyboard Shortcuts\033[0m\n"
...
export -f _horizon_git_list

_horizon_git_preview() {
    local -r hash="$1"
...
export -f _horizon_git_preview

_horizon_git_checkout() {
    ...
    if [[ ! -f "/tmp/horizon_time_machine_stash_${HORIZON_SESSION_ID}" ]]; then
        if ! git diff-index --quiet HEAD --; then
            ...
            if git stash push --quiet -m "HORIZON_AUTO_STASH_${HORIZON_SESSION_ID}"; then
                echo "STASHED" > "/tmp/horizon_time_machine_stash_${HORIZON_SESSION_ID}"
            else
            ...
            echo "CLEAN" > "/tmp/horizon_time_machine_stash_${HORIZON_SESSION_ID}"
        fi
    fi
...
export -f _horizon_git_checkout
...
_horizon_git_return() {
    ...
    if [[ -n "$main_branch" ]]; then
        ...
        if [[ -f "/tmp/horizon_time_machine_stash_${HORIZON_SESSION_ID}" ]]; then
            local status
            status=$(cat "/tmp/horizon_time_machine_stash_${HORIZON_SESSION_ID}")
            if [[ "$status" == "STASHED" ]]; then
                # Safe pop: If merge conflict occurs, aborts silently and keeps stash safe
                git stash pop --quiet || true
            fi
            rm -f "/tmp/horizon_time_machine_stash_${HORIZON_SESSION_ID}"
        fi
    fi
}
export -f _horizon_git_return

_horizon_git_restore() {
    exec < /dev/null > /dev/null 2>&1
    git reset --hard HEAD || true
}
export -f _horizon_git_restore
...
main() {
    ...
    _horizon_git_list | fzf --ansi \
        ...
        --layout=reverse \
        --border=rounded \
        --border-label=" 󰏖 Horizon Time Machine [F1 / Ctrl-O: Help] " \
        --border-label-pos=3 \
        --info=hidden \
        --header="$visual_header" \
        --header-first \
        --bind="enter:execute-silent(_horizon_git_checkout {1})+transform-prompt( [ -n \"{1}\" ] && echo \" :: Traveled to {1} ❯ \" || echo \" :: Time Machine ❯ \" )+reload-sync(_horizon_git_list)" \
        --bind="double-click:execute-silent(_horizon_git_checkout {1})+transform-prompt( [ -n \"{1}\" ] && echo \" :: Traveled to {1} ❯ \" || echo \" :: Time Machine ❯ \" )+reload-sync(_horizon_git_list)" \
        --bind="ctrl-r:execute-silent(_horizon_git_return)+change-prompt( :: Returned to Present ❯ )+reload-sync(_horizon_git_list)" \
        --bind="ctrl-w:execute-silent(_horizon_git_restore)+change-prompt( :: Restored (Hard Reset) ❯ )+reload-sync(_horizon_git_list)" \
        --bind="alt-c:execute-silent(_horizon_git_copy {1})+transform-prompt( [ -n \"{1}\" ] && echo \" :: Copied {1} ❯ \" || echo \" :: Time Machine ❯ \" )" \
        --bind="f1:execute(_horizon_git_help)" \
        --bind="ctrl-o:execute(_horizon_git_help)" \
        ...
        --preview="_horizon_git_preview {1}" \
