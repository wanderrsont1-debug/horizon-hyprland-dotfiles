...
CACHE_DIR: Final[Path] = _XDG_CACHE_HOME / "horizoncc"
SETTINGS_DIR: Final[Path] = _XDG_CONFIG_HOME / "horizon" / "settings"
...
def _sanitize_title(title: str | None) -> str:
    base = (title or "").strip() or "Horizon Terminal"
    sanitized = "".join(c if c.isprintable() and c not in "\n\r\t\x00" else " " for c in base)
    return " ".join(sanitized.split()) or "Horizon Terminal"
...
def _build_command_list(
    ...
    if requires_root:
        if run_in_terminal:
            return [
                "kitty", "--class", "horizon-term", "--title", safe_title,
...
class _SettingsWriteBuffer:
    ...
    def __new__(cls) -> _SettingsWriteBuffer:
        if cls._instance is None:
            ...
            inst._source_id = 0
            inst._executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="horizon-io-batch")
