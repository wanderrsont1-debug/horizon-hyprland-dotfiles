#!/usr/bin/env python3
"""
Backend module for Horizon Quick Panal.
...
APP_ID: Final = "org.horizon.quickpanal"
    ...
    def submit(self, func: Callable[..., Any], *args: Any, **kwargs: Any) -> Future[Any] | None:
        with self._lock:
            if self._executor is None:
                self._executor = ThreadPoolExecutor(max_workers=self._max_workers, thread_name_prefix="horizon-refresh")
...
def fetch_notifications() -> list[NotificationData]:
    ...
    ignored_apps = {"OSD", "horizon-keys", "horizon-cava", "horizon-cava-alert", "horizon-recorder", "horizon-tlp", "horizon-high-ram-alert", "Spotify", "matugen-theme", "horizon-fav-wal"}
    ...
    for src, items in [("history", history_items), ("active", active_items)]:
        for item in items:
            try:
                ...
                if app in ignored_apps or app.startswith("horizon-glance"): continue
