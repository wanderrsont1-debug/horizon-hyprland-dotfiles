#!/usr/bin/env python3
"""
Horizon Quick Panal: Configurable Main Execution
...
if sys.version_info < (3, 14, 5):
    raise SystemExit("Horizon Quick Panal requires Python 3.14.5 or newer.")
...
try:
    import ctypes
    _grab_lib_path = os.path.expanduser("~/user_scripts/horizon_system/click_away_to_dismiss/libwaylandgrab.so")
...
# ==============================================================================
CONFIG_DIR = Path(HOME) / ".config" / "horizon" / "quickpanal"
...
label = "Wi-Fi"
tooltip = "Wi-Fi\\nLMB: Network Manager"
on_left = "foot --app-id=horizon_tui python ~/user_scripts/horizon_tui/python/main/main.py ~/user_scripts/network_manager/tui_horizon_network.py"
...
label = "Updates"
tooltip = "Updates\\nLMB: System Update | RMB: Horizon Update"
on_left = "kitty --class system_update.sh --hold sh -c '~/user_scripts/update_horizon/system_update.sh --all'"
on_right = "kitty --class update_horizon.sh --hold sh -c '~/user_scripts/update_horizon/update_horizon.sh'"
...
class QuickPanalWindow(Gtk.ApplicationWindow):
    ...
    def _fetch_blur(self):
        ...
        try:
            with open(f"{HOME}/.config/horizon/settings/opacity_blur", "r") as f: state = f.read().strip().lower()
    ...
    def _fetch_power_profile(self):
        ...
        try:
            ...
            path = Path(HOME) / ".config" / "horizon" / "settings" / "tlp_state"
            if path.exists():
    ...
    def _fetch_updates(self):
        ...
        try:
            with open(f"{HOME}/.config/horizon/settings/waybar_update_counter_h", "r") as f: data = json.load(f)
    ...
    def _reposition_to_corner(self):
        try:
            ...
            win = next((c for c in clients if c.get("class") == "horizon_quickpanal.py"), None)
            if not win: return
