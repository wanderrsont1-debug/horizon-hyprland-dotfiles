#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# Horizon TUI Engine - Lua/Hyprland Refactor
...
declare CONFIG_FILE="${HORIZON_CONFIG_FILE:-${XDG_CONFIG_HOME}/hypr/hyprland.lua}"
declare -r APP_TITLE="Horizon Config Editor"
...
resolve_write_target() {
    ...
    local lock_dir="${XDG_RUNTIME_DIR:-/tmp}/horizon_tui_locks_${USER:-$UID}"
...
create_temp_near() {
    ...
    if ! REPLY=$(mktemp --tmpdir="$target_dir" ".${target_base}.${purpose}.XXXXXXXXXX" 2>/dev/null); then
        if ! REPLY=$(mktemp -t "horizon.${target_base}.${purpose}.XXXXXXXXXX" 2>/dev/null); then
            REPLY=""
            return 1
        fi
    fi
...
populate_config_cache() {
    ...
    tmp_proto=$(mktemp --tmpdir "horizon.parser.proto.XXXXXXXXXX") || { log_err "Failed to create parser IPC file"; return 1; }
    register_temp "$tmp_proto"
    tmp_err=$(mktemp --tmpdir "horizon.parser.err.XXXXXXXXXX") || {
...
local function unquote_string(raw)
    local chunk, err = load("return " .. raw, "=(horizon-key)", "t", {})
...
write_value_to_file() {
    ...
    err_file=$(mktemp --tmpdir "horizon.mutator.err.XXXXXXXXXX") || {
        release_lock_fd "$lock_fd"
    ...
    val_file=$(mktemp --tmpdir "horizon.value.XXXXXXXXXX") || {
