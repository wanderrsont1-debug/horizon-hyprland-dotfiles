    ...
    if _real_uid and _real_gid:
        try:
            ...
            def _fix_permissions():
                try:
                    for suffix_dir in [".config/horizon", ".cache/horizon_tui", "Documents/logs/tui", "Documents/horizon_backups/tui_reset"]:
...
def _setup_cache() -> None:
    try:
        ...
        cache_dir = xdg_cache / "horizon_tui"
...
SCHEMA_SEARCH_PATHS = [
    ...
    Path("~/.config/horizon_schema").expanduser().resolve(),
...
def setup_logging(module_name: str, enable_logging: bool) -> logging.Logger:
    ...
    logger = logging.getLogger("horizon_router")
...
def manage_backup(target_file: Path, action: str, logger: logging.Logger) -> bool:
    ...
    backup_dir = Path("~/Documents/horizon_backups/tui_reset/").expanduser()
...
EXAMPLES:
  ...
  4. Generate Markdown documentation for a schema:
    ...
    parser = argparse.ArgumentParser(
        description="Horizon TUI Master Router - Advanced Configuration Ecosystem",
    ...
    safe_module_namespace = f"horizon_schema_{module_name}"
    ...
    try:
        ...
        APP_TITLE = getattr(schema_module, "APP_TITLE", "Horizon Configurator")
    ...
    from python.frontend.ui import HorizonTUI
