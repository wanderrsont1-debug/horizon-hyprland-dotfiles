# Project Context & Handover Notes: Horizon VM Passthrough Setup

This workspace contains automated configuration scripts for setting up a GPU passthrough Windows 10/11 virtual machine (`win_10_horizon`) on Arch Linux with Looking Glass frame sharing.
...
- **Host OS**: Arch Linux
- **Host User**: `new`
- **Guest VM Name**: `win_10_horizon` (DNS name: `win_10_horizon`)
- **Guest IP Address**: `192.168.122.9`
- **Guest OS**: Windows 10
- **Guest SSH Credentials**: User `horizon`, Password `2345` (default shell configured to PowerShell)
...
All host-side scripts are located in `/home/new/user_scripts/horizon_vm/passthrough/`:
...
- Do not assume `000_horizon_vm.sh` orchestrator is used. Each Python/Shell/PowerShell script must be completely self-contained and idempotent.
