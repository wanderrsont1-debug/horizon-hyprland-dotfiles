#!/usr/bin/env bash
# ==============================================================================
# HORIZON GLANCE DAEMON - DYNAMIC WIDTH EDITION
# ==============================================================================

set -euo pipefail

SYNC_ID="horizon-glance-sync"
PID_FILE="${XDG_RUNTIME_DIR:-/run/user/$UID}/horizon_glance.pid"
...
if [[ -n "$MODE" && "$MODE" != "--stop" ]]; then
    CURRENT_APP="horizon-glance-${MODE#--}"
else
    CURRENT_APP="horizon-glance"
fi
...
case "$MODE" in
    ...
    --timer)
        ...
        while true; do
            ...
            if (( left <= 0 )); then
                # Leaving 'horizon-glance-alert' intact as requested via config overrides
                notify-send -u critical -a "horizon-glance-alert" -h string:x-canonical-private-synchronous:horizon-timer-alert "󰔛  Time's Up!"
        ...
        while true; do
            ...
            if (( left <= 0 )); then
                if [[ "$PHASE" == "WORK" ]] && (( BREAK_SEC > 0 )); then
                    notify-send -u critical -a "horizon-glance-alert" -h string:x-canonical-private-synchronous:horizon-timer-alert "󰦖  Break Time!"
                    ...
                    notify-send -u critical -a "horizon-glance-alert" -h string:x-canonical-private-synchronous:horizon-timer-alert "󰔚  $msg"
                    play_sound "/usr/share/sounds/freedesktop/stereo/alarm-clock-elapsed.oga"
