#!/usr/bin/env bash
# ==============================================================================
# HORIZON KEYS ELITE - ARCH LINUX / UV OPTIMIZED
# ==============================================================================
...
readonly APP_NAME="horizon_keys"
...
readonly NOT_SETUP_MSG="Install Horizon Keys from the Control Center."
...
notify_user() {
    if command -v notify-send >/dev/null 2>&1; then
        notify-send -u critical -t 5000 --app-name="horizon-keys" "Horizon Keys" "$1" || true
    fi
}
...
acquire_lock() {
    ...
    if ! flock -n "$LOCK_FD"; then
        printf "%b[TOGGLE]%b Horizon Keys is running. Shutting it down...\n" "${C_YELLOW}" "${C_RESET}"
        if [[ -f "$PID_FILE" ]]; then
            local pid
            pid=$(cat "$PID_FILE")
        ...
        if command -v notify-send >/dev/null 2>&1; then
            notify-send -u low -t 2000 --app-name="horizon-keys" "Horizon Keys" "Visualizer Disabled" || true
        fi
        exit 0
    fi
    LOCK_HELD=true
...
if [[ "$RUN_MODE" == "reset" ]]; then
    printf "%b[RESET]%b Cleaning Horizon Keys environment...\n" "${C_BLUE}" "${C_RESET}"
    if [[ -f "$PID_FILE" ]]; then
        pid=$(cat "$PID_FILE")
        kill -TERM "$pid" 2>/dev/null || true
    fi
...
SYNC_ID = "horizon-keys-sync"
APP_NAME = "horizon-keys"
DISPLAY_TIMEOUT = 3.0
POLL_INTERVAL = 1.0
...
async def main():
    ...
    monitored_tasks = {}
    print("Horizon Keys engine started...")
...
printf "%b[RUN]%b Starting Horizon Keys background daemon...\n" "${C_BLUE}" "${C_RESET}"
...
if command -v notify-send >/dev/null 2>&1; then
    notify-send -u low -t 2000 --app-name="horizon-keys" "Horizon Keys" "Visualizer Enabled" || true
fi
