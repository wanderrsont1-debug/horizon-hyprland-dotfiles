...
#    "U | gsettings set org.cinnamon.desktop.default-applications.terminal exec 'kitty'"
    ...
    "U | systemctl --user disable --now horizon_sliders.service || true"
    # --- Remove old horizon_snaapshot timer (typo) before re-deploying horizon_snapshot ---
    "S | systemctl stop horizon_snaapshot.timer horizon_snaapshot.service 2>/dev/null; systemctl disable horizon_snaapshot.timer 2>/dev/null; true"
    "S | rm -f /etc/systemd/system/horizon_snaapshot.service /etc/systemd/system/horizon_snaapshot.timer"
