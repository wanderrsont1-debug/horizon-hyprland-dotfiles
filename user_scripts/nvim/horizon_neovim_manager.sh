...
readonly HORIZON_SRC="${XDG_CONFIG_HOME:-$HOME/.config}/horizon_nvim"
...
print_help() {
    clear || true
    printf "${BOLD}====================================================${RESET}\n"
    printf "${BOLD}               Horizon Neovim Manager                 ${RESET}\n"
    printf "${BOLD}====================================================${RESET}\n\n"
    ...
    printf "  ${GREEN}-t, --target${RESET}    Specify installation target (nvchad, lazyvim, astronvim, horizon).\n"
    printf "\n"
...
install_horizon_nvim() {
    log_info "Deploying Horizon Neovim..."
    if [[ ! -d "${HORIZON_SRC}" ]]; then
        log_error "Horizon Neovim source not found at ${HORIZON_SRC}"
        exit 1
    fi
    mkdir -p "${NVIM_PATHS[config]}"
    cp -a "${HORIZON_SRC}/." "${NVIM_PATHS[config]}/"
    log_success "Horizon Neovim deployed precisely."
}
...
main() {
    ...
    if [[ "${AUTONOMOUS_MODE}" == "true" ]]; then
        ...
        case "${INSTALL_TARGET,,}" in
            ...
            horizon)     install_horizon_nvim ;;
            *)
                log_error "Invalid target: ${INSTALL_TARGET}. Valid options: nvchad, lazyvim, astronvim, horizon."
                trap - INT TERM EXIT
                exit 1
                ;;
        esac
        ...
        select nvim_config in "Install NvChad" "Install LazyVim" "Install AstroNvim" "Install Horizon Neovim" "Maintenance & Utilities" "Quit"; do
            case "${REPLY}" in
                ...
                4) prompt_state_management; install_horizon_nvim; action_taken=true; break ;;
                5) 
                    echo ""
                    local main_ps3="${PS3}"
