#!/usr/bin/env bash
# =============================================================================
# Horizon Package Atlas (Platinum Edition - Revision 19 - Centered Apex)
...
'
    ...
    export HORIZON_PKG_COPY='
export LC_ALL=C
pkg="$1"
bash -c "$HORIZON_PKG_PREVIEW" _ "$pkg" | sed -r "s/\x1B\[([0-9]{1,3}(;[0-9]{1,2})?)?[mGK]//g" | wl-copy
'
    ...
    local fzf_choice
    fzf_choice=$(bash -c "$HORIZON_PKG_LIST" _ "$init_mode" "$init_target" | fzf --ansi \
        --delimiter='\|' \
        --with-nth=2 \
        ...
        --layout=reverse \
        --border=rounded \
        --border-label=" 󰏖 Horizon Package Atlas [F1: Help] " \
        --border-label-pos=3 \
        --info=hidden \
        --header="$visual_header" \
        --header-first \
        --bind="ctrl-s:reload-sync(bash -c \"\$HORIZON_PKG_LIST\" _ size_desc $init_target)+change-prompt(   Hogs (Largest) ❯ )" \
        --bind="alt-s:reload-sync(bash -c \"\$HORIZON_PKG_LIST\" _ size_asc $init_target)+change-prompt(   Tiny (Smallest) ❯ )" \
        --bind="ctrl-d:reload-sync(bash -c \"\$HORIZON_PKG_LIST\" _ date_desc $init_target)+change-prompt(   Newest (Date) ❯ )" \
        --bind="ctrl-r:reload-sync(bash -c \"\$HORIZON_PKG_LIST\" _ alpha $init_target)+change-prompt(   Search Packages ❯ )" \
        --bind="f1:execute(bash -c \"\$HORIZON_PKG_HELP\")" \
        --bind="alt-c:execute-silent(bash -c \"\$HORIZON_PKG_COPY\" _ {1})+change-prompt(   Copied Info! ❯ )" \
        ...
        --preview='bash -c "$HORIZON_PKG_PREVIEW" _ {1}' \
        --preview-window="right,50%,border-left,wrap")

    # Environment Cleanup
    unset HORIZON_PKG_LIST
    unset HORIZON_PKG_PREVIEW
    unset HORIZON_PKG_COPY
    unset HORIZON_PKG_HELP
