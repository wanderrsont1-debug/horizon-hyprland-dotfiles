...
tui_root = Path(__file__).resolve().parents[2] / "horizon_tui"
...
APP_TITLE = "Horizon CPU Core Manager"
DEFAULT_MODE = "auto"
...
if __name__ == "__main__":
    ...
    def display_status_table():
        ...
        console = Console()
        console.print(Align.center(Panel("[bold magenta]Horizon CPU Core Manager[/bold magenta]", border_style="cyan", expand=False)))
    ...
    # Check if we should delegate to horizon_tui main.py
    ...
    if len(sys.argv) == 1 or any(arg in delegate_flags for arg in sys.argv):
        main_py = Path(__file__).resolve().parents[2] / "horizon_tui" / "python" / "main" / "main.py"
        cmd = [sys.executable, str(main_py), str(Path(__file__).resolve()), *sys.argv[1:]]
        try:
            res = subprocess.run(cmd)
            sys.exit(res.returncode)
        except Exception as e:
            print(f"[-] Error delegating to horizon_tui: {e}")
            sys.exit(1)
