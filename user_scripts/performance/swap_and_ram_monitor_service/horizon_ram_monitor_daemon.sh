#!/usr/bin/env bash
# Horizon RAM Monitor - Pure Bash Real-Time Memory HUD Daemon
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Horizon RAM HUD started. Config: CriticalRAM=${THRESHOLD_RAM_CRITICAL}%, HighRAM=${THRESHOLD_RAM_HIGH}%, HighZRAM=${THRESHOLD_ZRAM_HIGH}%, RecoveryRAM=${THRESHOLD_RAM_RECOVERY}%, PollInterval=${POLL_INTERVAL}s" >&2
...
while true; do
    ...
    if (( EPOCHSECONDS < snooze_expire_time )); then
        ...
        if [[ "$hud_active" == true ]]; then
            ...
            /usr/bin/notify-send -a "horizon-high-ram-alert" \
                -h string:x-canonical-private-synchronous:horizon-ram-hud \
                -t 1 " " " "
                
            # Step B: Spawn fresh, independent green recovery notification
            /usr/bin/notify-send -a "horizon-ram-recovered" \
                -u normal \
                -t 3000 \
                "SYSTEM RECOVERED" \
