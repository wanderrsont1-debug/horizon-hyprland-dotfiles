#!/usr/bin/env bash
# ~/user_scripts/rofi/horizon_launcher.sh
# Unified All-in-One Launcher & Data Provider

# ==============================================================================
# 1. DATA PROVIDER MODE (Populates the "Horizon" tab)
...
if [[ "$1" == "--rofi-mode" ]]; then
    ...
    if [[ -z "$ROFI_RETV" || "$ROFI_RETV" -eq 0 ]]; then
        ...
        for file in ~/.local/share/applications/*horizon*.desktop /usr/share/applications/*horizon*.desktop; do
...
rofi -show combi \
     -modes "drun,combi,Horizon:${SCRIPT_PATH} --rofi-mode" \
     -combi-modes "drun,Horizon" \
     -combi-hide-mode-prefix true \
     -display-drun "󰀻 Apps" \
     -display-combi "󰜉 All" \
     -display-Horizon "󰒓 Horizon" \
