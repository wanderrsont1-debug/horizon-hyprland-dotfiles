...
FIFO_PATH = Path("/tmp/horizon_kokoro.fifo")
PID_FILE = Path("/tmp/horizon_kokoro.pid")
READY_FILE = Path("/tmp/horizon_kokoro.ready")
LOCK_FILE = Path("/tmp/horizon_kokoro.lock")

# --- VOICE SETUP ---
# Handled safely by Horizon TUI. Do not use multi-line dicts here.
BLEND_VOICES = True
VOICE_1 = "af_heart"
VOICE_1_WEIGHT = 0.4
VOICE_2 = "af_bella"
...
# LOGGING
# ==============================================================================
logger = logging.getLogger("horizon_daemon")
...
class HorizonDaemon:
    def __init__(self, debug_file=None):
        ...
        logger.info(f"Horizon Daemon {VERSION} Initializing...")
    ...
    def _acquire_instance_lock(self):
        ...
        try:
            ...
            self._lock_fd = None
            raise RuntimeError("Another Horizon daemon instance is already running.")
...
if __name__ == "__main__":
    ...
    log_level = os.environ.get("HORIZON_LOG_LEVEL", args.log_level).upper()
    if hasattr(logging, log_level):
        logger.setLevel(getattr(logging, log_level))

    debug_path = args.debug_file or os.environ.get("HORIZON_LOG_FILE")

    if args.daemon:
        HorizonDaemon(debug_path).start()
    else:
