...
# ==============================================================================
# HORIZON KOKORO INSTALLER V36 (AMD Fix + Model Precision + Final Polish)
# ==============================================================================

readonly SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
readonly ENV_DIR="$HOME/contained_apps/uv/horizon_kokoro"
readonly MODEL_DIR="$ENV_DIR/models"
readonly TRIGGER_DIR="$HOME/user_scripts/tts_stt/horizon_kokoro"
...
echo ":: [V36] Initializing Horizon Kokoro Setup..."
...
if [[ -f "$SCRIPT_DIR/horizon_main.py" ]]; then
    cp "$SCRIPT_DIR/horizon_main.py" "$ENV_DIR/"
    echo ":: horizon_main.py deployed."
else
    echo ":: ERROR: horizon_main.py not found in current directory."
    exit 1
fi
...
#!/usr/bin/env bash
# Horizon Kokoro Trigger V36 ($MODE edition)
# Features: Universal HW, Robust Detection, Cold Boot Fix, Hard Kill, Base64 IPC

readonly APP_DIR="$HOME/contained_apps/uv/horizon_kokoro"
readonly PID_FILE="/tmp/horizon_kokoro.pid"
readonly READY_FILE="/tmp/horizon_kokoro.ready"
readonly FIFO_PATH="/tmp/horizon_kokoro.fifo"
readonly DAEMON_LOG="/tmp/horizon_kokoro.log"
readonly DEBUG_LOG="\$APP_DIR/horizon_debug.log"
...
start_daemon() {
    ...
    if [[ "\$debug_mode" == "true" ]]; then
        ...
        export HORIZON_LOG_LEVEL="DEBUG"
        export HORIZON_LOG_FILE="\$DEBUG_LOG"
        nohup uv run horizon_main.py --daemon --debug-file "\$DEBUG_LOG" > "\$DAEMON_LOG" 2>&1 &
    else
        nohup uv run horizon_main.py --daemon > "\$DAEMON_LOG" 2>&1 &
    fi
...
show_help() {
    cat << 'HELP'
Horizon Kokoro TTS — Trigger Script
