...
# ==============================================================================
VERSION = "1.2 (Horizon STT - Explicit Caching & Wayland Optimized)"

ZRAM_MOUNT = Path("/tmp") # Using /tmp for audio buffering
FIFO_PATH = Path("/tmp/horizon_stt.fifo")
PID_FILE = Path("/tmp/horizon_stt.pid")
READY_FILE = Path("/tmp/horizon_stt.ready")
...
# LOGGING
# ==============================================================================
logger = logging.getLogger("horizon_stt_daemon")
...
# ==============================================================================
class HorizonSTTDaemon:
    def __init__(self, debug_file=None, device_mode="cpu"):
        self.running = True
        self.device_mode = device_mode
        if debug_file: setup_debug_logging(debug_file)
        
        logger.info(f"Horizon STT Daemon {VERSION} Initializing [Mode: {self.device_mode.upper()}]...")
...
if __name__ == "__main__":
    ...
    log_level = os.environ.get("HORIZON_STT_LOG_LEVEL", args.log_level).upper()
    if hasattr(logging, log_level): logger.setLevel(getattr(logging, log_level))
    debug_path = args.debug_file or os.environ.get("HORIZON_STT_LOG_FILE")
    
    if args.daemon: HorizonSTTDaemon(debug_path, args.mode).start()
