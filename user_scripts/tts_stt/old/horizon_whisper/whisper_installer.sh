...
echo ":: Initializing Horizon STT Setup..."
...
if [[ -f "$SCRIPT_DIR/horizon_stt_main.py" ]]; then
    cp "$SCRIPT_DIR/horizon_stt_main.py" "$ENV_DIR/"
    echo ":: horizon_stt_main.py deployed."
else
    echo ":: ERROR: horizon_stt_main.py not found in current directory."
    exit 1
fi
...
#!/usr/bin/env bash
# Horizon STT Trigger ($MODE edition)
# Toggle behavior: First run starts recording. Second run stops recording and transcribes.

readonly APP_DIR="$HOME/contained_apps/uv/horizon_stt"
readonly PID_FILE="/tmp/horizon_stt.pid"
readonly READY_FILE="/tmp/horizon_stt.ready"
readonly FIFO_PATH="/tmp/horizon_stt.fifo"
readonly DAEMON_LOG="/tmp/horizon_stt.log"
readonly DEBUG_LOG="\$APP_DIR/horizon_stt_debug.log"
readonly INSTALL_MODE="$MODE"

# Recording vars
readonly RECORD_PID_FILE="/tmp/horizon_stt_record.pid"
readonly YAD_PID_FILE="/tmp/horizon_stt_yad.pid"
readonly AUDIO_TMP_FILE="/tmp/horizon_stt_capture.wav"
...
start_daemon() {
    ...
    if [[ "\$debug_mode" == "true" ]]; then
        export HORIZON_STT_LOG_LEVEL="DEBUG"
        export HORIZON_STT_LOG_FILE="\$DEBUG_LOG"
        nohup uv run horizon_stt_main.py --daemon --mode "\$INSTALL_MODE" --debug-file "\$DEBUG_LOG" > "\$DAEMON_LOG" 2>&1 &
    else
        nohup uv run horizon_stt_main.py --daemon --mode "\$INSTALL_MODE" > "\$DAEMON_LOG" 2>&1 &
    fi
...
if [[ -f "\$RECORD_PID_FILE" ]] && kill -0 "\$(cat "\$RECORD_PID_FILE")" 2>/dev/null; then
    ...
    pkill -f "yad --title=Horizon STT" 2>/dev/null || true
    ...
    (
        ...
        if [[ -f "\$RECORD_PID_FILE" ]] && kill -0 "\$(cat "\$RECORD_PID_FILE")" 2>/dev/null; then
            if [ \$yad_exit -eq 0 ]; then
                ...
                notify -t 1500 "Horizon STT" "Recording Cancelled."
            fi
        fi
    ) &
    YAD_PID=\$!
