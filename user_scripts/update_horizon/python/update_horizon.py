#!/usr/bin/env python3
# ==============================================================================
#  HORIZON UPDATER (v9.4.1) — BLEEDING EDGE ARCH / PYTHON 3.14 TUI
# ==============================================================================
import asyncio
import json
import shutil
...
if sys.version_info < (3, 14):
    sys.stdout.write("\033[1;31m[FATAL]\033[0m Horizon requires Python 3.14+ bleeding-edge architecture.\n")
    sys.exit(1)
...
def verify_sudo() -> bool:
    sys.stdout.write("\033[1;36m[HORIZON PRE-FLIGHT]\033[0m Securing administrative kernel privileges...\n")
    try:
...
def bootstrap_dependencies() -> bool:
    ...
    if missing:
        sys.stdout.write(f"\033[1;33m[HORIZON BOOTSTRAP]\033[0m Resolving dependencies: {', '.join(missing)}\n")
...
def compile_theme() -> dict[str, str]:
    ...
    theme_path = Path.home() / ".config/matugen/generated/horizon_tui.json"
...
# ==============================================================================
WORK_TREE = Path.home()
GIT_DIR = WORK_TREE / "horizon"
BACKUP_BASE_DIR = WORK_TREE / "Documents" / "horizon_backups"

REPO_URL = "https://github.com/dusklinux/horizon"
BRANCH = "main"
...
SCRIPT_SEARCH_DIRS = [
    ...
    "user_scripts/tts_stt/horizon_kokoro",
    "user_scripts/tts_stt/horizon_parakeet",
    "user_scripts/networking",
    "user_scripts/misc_extra",
    "user_scripts/misc_extra/delete_in_3_weeks",
    "user_scripts/update_horizon/update_checker",
    "user_scripts/update_horizon",
    "user_scripts/horizon_system/reload_cc",
...
UPDATE_SEQUENCE = [
 ...
 #   "U | cc_restart.sh --quiet",
    ...
    "U | ignore-fail | interactive | horizon_matugen_config_tui.sh --smart",
...
def parse_manifest(sequence: list[str]) -> list[HorizonTask]:
    tasks = [
        HorizonTask("Git Bare Repo Validation", 'GIT', False, False, []),
        HorizonTask("Fetch Upstream & Diff", 'GIT', False, False, []),
        HorizonTask("Forensic Collision Backup", 'GIT', False, False, []),
        HorizonTask("Atomic Snapshot (CoW)", 'GIT', False, False, []),
        HorizonTask("Apply Bare Updates (Reset)", 'GIT', False, False, [])
    ]

    interactive_heuristics = {'reboot_post_lua_update.sh', 'horizon_matugen_config_tui.sh', 'horizon_firefox_tui.sh'}
...
class MainLogItem(ListItem):
    def compose(self) -> ComposeResult:
        yield Label(f" [bold {THEME['accent']}]CORE[/] Horizon Execution Engine", classes="list-item-label")


class TaskItem(ListItem):
    status = reactive("pending")

    def __init__(self, task: HorizonTask, index: int):
        super().__init__()
        self.horizon_task = task
    ...
    def _update_label(self) -> None:
        ...
        if self.horizon_task.mode == 'GIT':
            badge = f"[bold {THEME['accent']}]GIT[/]"
        elif self.horizon_task.mode == 'S':
            badge = f"[bold {THEME['error']}]SUDO[/]"
        else:
            badge = f"[bold {THEME['success']}]USER[/]"
        
        cmd_str = f"{self.horizon_task.name} {' '.join(self.horizon_task.args)}".strip()
        if len(cmd_str) > 31: cmd_str = cmd_str[:28] + "..."
        cmd_str = escape(cmd_str)

        suffix = ""
        if self.horizon_task.name == "Fetch Upstream & Diff" and getattr(self.app, 'git_diff_text', None) and self.status in ("success", "skipped"):
...
class HorizonApp(App):
    ...
    def __init__(self, tasks: list[HorizonTask], has_sudo: bool):
        super().__init__()
        self.tasks = tasks
    ...
    def compose(self) -> ComposeResult:
        yield Static(" 🦅 HORIZON PIPELINE ENGINE (v9.4.1 — Elegance Edition)", classes="header-panel")
            ...
            try:
                if task.interactive:
                    ...
                    with self.suspend():
                        ...
                        sys.stdout.write(f"\n\033[1;38;2;{r};{g};{b}m=== HORIZON INTERACTIVE ABSTRACTION: {task.name} ===\033[0m\n\n")
                        sys.stdout.flush()
