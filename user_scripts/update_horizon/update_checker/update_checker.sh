#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# Horizon Git Checker & TUI Viewer
# -----------------------------------------------------------------------------
...
declare -r GIT_DIR="${HOME}/horizon/"
declare -r WORK_TREE="${HOME}"
declare -r STATE_FILE="${HOME}/.config/horizon/settings/horizon_update_behind_commit"
...
declare -r LOCK_BASENAME="horizon_git_fetch.${UID}.lock"

# TUI settings
declare -r APP_TITLE="Horizon Updates"
...
write_state_file() {
    ...
    tmp=$(/usr/bin/mktemp --tmpdir="$STATE_DIR" '.horizon_update_behind_commit.XXXXXX') || return 1
...
load_commits() {
    ...
    if (( count == 0 )); then
        COMMIT_HASHES=("HEAD")
        COMMIT_MSGS=("Horizon is up to date!")
        TOTAL_COMMITS=1
        return
    fi
