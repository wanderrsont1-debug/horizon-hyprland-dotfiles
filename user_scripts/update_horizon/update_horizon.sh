#!/usr/bin/env bash
# ==============================================================================
#  HORIZON UPDATER (v8.0) — ARCHITECTURE & USAGE MANUAL
# ==============================================================================
...
# CONFIGURATION
# ==============================================================================
declare -r DOTFILES_GIT_DIR="${HOME}/horizon"
declare -r WORK_TREE="${HOME}"
declare -r LOG_BASE_DIR="${HOME}/Documents/logs"
declare -r BACKUP_BASE_DIR="${HOME}/Documents/horizon_backups"
declare -r STATE_HOME_DIR="${XDG_STATE_HOME:-${HOME}/.local/state}/horizon"
declare -r FALLBACK_LOG_BASE_DIR="${STATE_HOME_DIR}/logs"
declare -r FALLBACK_BACKUP_BASE_DIR="${STATE_HOME_DIR}/backups"
declare -r REPO_URL="https://github.com/dusklinux/horizon"
declare -r BRANCH="main"
declare -r UPSTREAM_REMOTE="horizon-upstream"
declare -r UPSTREAM_TRACKING_REF="refs/horizon-updater/upstream/${BRANCH}"
...
declare -a SCRIPT_SEARCH_DIRS=(
    ...
    "${WORK_TREE}/user_scripts/update_horizon/update_checker"
    "${WORK_TREE}/user_scripts/horizon_system/reload_cc"
    "${WORK_TREE}/user_scripts/services"
    "${WORK_TREE}/user_scripts/update_horizon"
    ...
    "${WORK_TREE}/user_scripts/tts_stt/horizon_kokoro"
    "${WORK_TREE}/user_scripts/tts_stt/horizon_parakeet"
)
...
#    "U | cc_restart.sh --quiet"
    ...
    "S | horizon_service_manager.sh"
#    "U | append_defaults_keybinds_edit_here.sh"
    "U | ignore-fail | horizon_matugen_config_tui.sh --smart"
#    "U | ignore-fail | horizon_firefox_tui.sh --sync --all"
    "U | ignore-fail | hypr_anim.sh --current"
    "U | ignore-fail | theme_ctl.sh refresh"
    "U | ignore-fail | update_counter.sh"
    "U | horizon_commands_after.sh"
...
desktop_notify() {
    ...
    local summary="${2:-Horizon Update}"
    local body="${3:-}"

    if command -v notify-send &>/dev/null; then
        timeout 3 notify-send --urgency="$urgency" --app-name="Horizon Updater" "$summary" "$body" \
            >/dev/null 2>&1 || true
    fi
}
...
show_help() {
    cat <<'HELPEOF'
Horizon Updater — Dotfile sync and setup tool for Arch Linux / Hyprland

Usage: horizon_updater.sh [OPTIONS]
...
Logs are saved to:
  ~/Documents/logs/
  Fallback: ~/.local/state/horizon/logs/

Backups are saved to:
  ~/Documents/horizon_backups/
  Fallback: ~/.local/state/horizon/backups/
HELPEOF
}

show_version() {
    printf 'Horizon Updater v%s\n' "$VERSION"
}
...
setup_runtime_dir() {
    ...
    if [[ -n "${XDG_RUNTIME_DIR:-}" ]]; then
        candidate="${XDG_RUNTIME_DIR}/horizon-updater"
    ...
    candidate="/tmp/horizon-updater-${EUID}"
    if ! ensure_secure_runtime_dir "$candidate"; then
...
setup_logging() {
    LOG_FILE="$(make_private_file_under "$ACTIVE_LOG_BASE_DIR" "horizon_update_${RUN_TIMESTAMP}_XXXXXX.log")" || {
...
auto_prune() {
    if [[ -d "$ACTIVE_LOG_BASE_DIR" ]]; then
        find "$ACTIVE_LOG_BASE_DIR" -type f -name 'horizon_update_*.log' -mtime "+${LOG_RETENTION_DAYS}" -delete \
            2>/dev/null || true
    fi
...
backup_worktree_collisions_for_ref() {
    ...
    {
        printf 'Horizon work-tree collision backup\n'
        printf 'Created: %s\n' "$RUN_TIMESTAMP"
        printf 'Reference: %s\n' "$ref"
        printf 'Work tree: %s\n' "$WORK_TREE"
    } > "$info_file" || {
...
handle_unrelated_upstream_history() {
    ...
    if [[ -t 0 ]]; then
        printf '\n%s[UNRELATED HISTORY]%s The existing bare repo at %s is not based on Horizon upstream.\n' \
...
backup_full_tracked_tree() {
    ...
    printf 'Horizon full tracked-tree backup\n' >> "$info_file"
...
backup_git_history() {
    ...
    {
        printf 'Horizon Git history backup\n'
        printf 'Created: %s\n' "$RUN_TIMESTAMP"
        printf 'Source: %s\n' "$DOTFILES_GIT_DIR"
    } > "$info_file" || true
    chmod 600 -- "$info_file" 2>/dev/null || true
...
atomic_restore_path() {
    ...
    tmpdir="$(mktemp -d -p "$parent" ".${base}.horizon_tmp.XXXXXX")" || return 1
    ...
    for path in "${CHANGE_PATHS[@]}"; do
        ...
        case "$action" in
            ...
            delete-merge)
                ...
                marker="${MERGE_DIR}/${path}.horizon_deleted"
                ensure_relative_parent_dir "$MERGE_DIR" "${path}.horizon_deleted" mkdir_cache || {
...
cleanup() {
    ...
    if ((${#HARD_FAILED_SCRIPTS[@]} > 0)); then
        desktop_notify critical "Horizon Update" "${#HARD_FAILED_SCRIPTS[@]} required script(s) failed"
        exit 1
    elif ((rc != 0)); then
        desktop_notify critical "Horizon Update" "Update failed or interrupted"
        exit "$rc"
    elif [[ "$SYNC_FAILED" == true ]]; then
        desktop_notify critical "Horizon Update" "Sync phase failed"
        exit 1
    else
        desktop_notify normal "Horizon Update" "Update completed successfully"
        exit 0
    fi
}
